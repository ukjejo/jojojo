...
welfare = read.spss("./csv1/Koweps_hpc10_2015_beta1.sav", to.data.frame = T)
View(welfare)
...
welfare %>% select(gender,birth,marriage,religion,
...
welfare_copy %>% 
  filter(!is.na(income)) %>% 
  group_by(age) %>% 
  summarise(income_mean_age = mean(income)) -> age_income

# 시각화
ggplot(data = age_income,
       aes(x = age, y = income_mean_age)) + geom_line()

age_income %>% arrange(desc(income_mean_age)) %>% head(5)
...
welfare_copy$ageg = welfare_copy$age  

ifelse(welfare_copy$ageg >= 60, 'old',
       ifelse(welfare_copy$ageg >= 30,  'middle', 'young')) -> welfare_copy$ageg

welfare_copy %>% 
  filter(!is.na(income)) %>%
  group_by(ageg) %>% 
  summarise(income_mean_ageg = mean(income)) -> income_ageg 

ggplot(data = income_ageg,
       aes(x = ageg, y = income_mean_ageg)) + geom_col() + 
...
welfare_copy %>% 
  filter(!is.na(income)) %>% 
  group_by(age,gender) %>% 
  summarise(income_mean = mean(income)) -> income_age_gender

ggplot(data = income_age_gender,
       aes(x= age, y = income_mean, color = gender)) +
  geom_line()

# 직업별로 평균 월급 조회

list_job = read_excel("./csv1/Koweps_Codebook.xlsx",sheet = 2, col_names = T)
...
ggplot(head(job_income,10),
       aes(x = reorder(job, income_mean), y = income_mean)) + geom_col() +
  coord_flip() 

ggplot(tail(job_income,10),
       aes(x = reorder(job, income_mean), y = income_mean)) + geom_col() +
  coord_flip() 
...
### marriage : 3인 경우 이혼
# 새로운 파생변수 생성하여 (group_marriage)
# marriage가 3이면 divorce
# marriage가 1이면 marriage
# 연령대 별 이혼율 출력 시각화

welfare_copy$group_marriage = welfare_copy$marriage

ifelse(welfare_copy$group_marriage == 3, 'divorce',
       ifelse(welfare_copy$group_marriage == 1, 'marriage', NA)) -> welfare_copy$group_marriage
